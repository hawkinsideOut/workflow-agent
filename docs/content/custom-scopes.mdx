...
```
✓ 12 scopes validated successfully
...
└── src/
    ...
    └── index.test.ts  # Test suite (generated automatically)
```
...
workflow scope:create \
  ...
  --scopes "accounts:Account management:👤:features,payments:Payment processing:💳:features,transactions:Transaction history:📊:features"
```
...
```
✓ src/index.test.ts (5)
  ✓ FinTech Platform Scope Preset (5)
    ✓ should export valid scopes array
    ✓ should have valid preset object
    ✓ should have no duplicate scope names
    ✓ should have all scopes match schema
...
🎯 **Aim for 8-15 scopes per preset**
...
```typescript
export const scopes: Scope[] = [
  { name: 'patients', description: 'Patient records and profiles', emoji: '👤' },
  { name: 'appointments', description: 'Scheduling and calendar', emoji: '📅' },
  { name: 'prescriptions', description: 'Prescription management', emoji: '💊' },
  { name: 'lab-results', description: 'Laboratory test results', emoji: '🧪' },
  { name: 'billing', description: 'Medical billing and insurance', emoji: '💳' },
  { name: 'messaging', description: 'Secure patient messaging', emoji: '💬' },
  { name: 'reports', description: 'Medical reports and analytics', emoji: '📊' },
  { name: 'compliance', description: 'HIPAA and regulatory compliance', emoji: '🔒' },
];
```
...
```typescript
export const scopes: Scope[] = [
  { name: 'players', description: 'Player accounts and profiles', emoji: '🎮' },
  { name: 'matches', description: 'Game matches and sessions', emoji: '🏆' },
  { name: 'leaderboards', description: 'Rankings and leaderboards', emoji: '📊' },
  { name: 'inventory', description: 'In-game items and inventory', emoji: '🎒' },
  { name: 'marketplace', description: 'Item trading and marketplace', emoji: '🏪' },
  { name: 'chat', description: 'Player communication and chat', emoji: '💬' },
  { name: 'guilds', description: 'Clans and guild management', emoji: '⚔️' },
  { name: 'events', description: 'Tournaments and special events', emoji: '🎯' },
];
```

## Next Steps

- ✅ **[View existing presets](/docs/presets)** to see if one fits your needs
- ✅ **[Learn about configuration](/docs/configuration)** to customize enforcement
- ✅ **[Create your first scope package](#quick-start)** with `workflow scope:create`
- ✅ **[Migrate existing scopes](#migrating-existing-scopes)** with `workflow scope:migrate`
