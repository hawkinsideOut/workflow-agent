...
export class ValidationProvider {
  ...
  constructor(private configManager: ConfigManager) {
    this.diagnosticCollection = vscode.languages.createDiagnosticCollection('workflow-agent');
  }
    ...
    if (!commitPattern.test(firstLine)) {
      ...
      if (match) {
        ...
        if (validScopes.length > 0 && !validScopes.includes(scope)) {
          const diagnostic = new vscode.Diagnostic(
            new vscode.Range(0, firstLine.indexOf(scope), 0, firstLine.indexOf(scope) + scope.length),
