...
describe('workflow hooks - E2E', () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        scopes: [{ name: 'feat', description: 'New features and enhancements' }],
        enforcement: 'strict',
        language: 'en',
      })
    );
  ...
  describe('hooks install', () => {
    it('successfully installs hooks in empty hooks directory', async () => {
      const { exitCode, stdout } = await execa('node', [cliPath, 'hooks', 'install'], {
        cwd: tempDir,
        reject: false,
      });
      ...
      const preCommit = await readFile(join(tempDir, '.git', 'hooks', 'pre-commit'), 'utf-8');
    ...
    it('creates wrapper when existing hook is present', async () => {
      ...
      const backup = await readFile(join(hooksDir, 'pre-commit.original'), 'utf-8');
  ...
  describe('hooks uninstall', () => {
    it('removes installed hooks', async () => {
      ...
      // Then uninstall
      const { exitCode, stdout } = await execa('node', [cliPath, 'hooks', 'uninstall'], {
        cwd: tempDir,
        reject: false,
      });
  ...
  describe('hooks status', () => {
    it('reports not installed when hooks are missing', async () => {
      const { stdout, exitCode } = await execa('node', [cliPath, 'hooks', 'status'], {
        cwd: tempDir,
        reject: false,
      });
  ...
  describe('without git repository', () => {
    it('fails gracefully when no git repo exists', async () => {
      ...
      try {
        const { exitCode, stderr } = await execa('node', [cliPath, 'hooks', 'install'], {
          cwd: noGitDir,
          reject: false,
        });
