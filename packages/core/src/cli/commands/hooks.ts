  ...
  for (const result of results) {
    if (result.success) {
      if (result.wrappedExisting) {
        console.log(chalk.green(`✓ Installed ${result.hookType} hook (wrapped existing hook)`));
      } else {
        console.log(chalk.green(`✓ Installed ${result.hookType} hook`));
      }
    } else {
      console.error(chalk.red(`✗ Failed to install ${result.hookType}: ${result.error}`));
      hasErrors = true;
    }
  }
...
async function uninstallHooksAction(cwd: string) {
  ...
  for (const result of results) {
    if (result.success) {
      if (result.wrappedExisting) {
        console.log(chalk.green(`✓ Removed ${result.hookType} hook (restored original)`));
      } else {
        console.log(chalk.green(`✓ Removed ${result.hookType} hook`));
      }
    } else if (result.error) {
      console.error(chalk.red(`✗ Failed to remove ${result.hookType}: ${result.error}`));
      hasErrors = true;
    }
  }
