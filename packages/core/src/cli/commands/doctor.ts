...
export async function doctorCommand(options: DoctorOptions = {}) {
  ...
  if (!hasGitRepo(cwd)) {
    ...
    if (installedHooks.length === hookStatuses.length) {
      console.log(
        chalk.green(`  ✓ All ${installedHooks.length} hooks installed`),
      );
  ...
  if (repoInfo.isGitHub && hasCIWorkflow(cwd)) {
    ...
    console.log(
      chalk.dim("    ☑ Require status checks to pass before merging"),
    );
    console.log(
      chalk.dim("    ☑ Require branches to be up to date before merging"),
    );
  }
  ...
  if (config.scopes.length < 5) {
    console.log(
      chalk.cyan("  → Consider adding more scopes for better organization"),
    );
