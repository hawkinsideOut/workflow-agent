...
async function setupAction(cwd: string) {
  ...
  if (result.success) {
    ...
    for (const check of checks) {
      const hasScript =
        check === "lint"
          ? projectInfo.hasLintScript
          : check === "typecheck"
            ? projectInfo.hasTypecheckScript
            : check === "format"
              ? projectInfo.hasFormatScript
              : check === "test"
                ? projectInfo.hasTestScript
                : check === "build"
                  ? projectInfo.hasBuildScript
                  : false;
    ...
    console.log(
      chalk.dim('  Enable "Require status checks to pass before merging"'),
    );
