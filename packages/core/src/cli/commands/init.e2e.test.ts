...
describe('workflow init - E2E', () => {
  ...
  describe('non-interactive mode', () => {
    it('initializes project with preset and name flags', async () => {
      ...
      const { exitCode } = await execa('node', [cliPath, 'init', '--preset', 'saas', '--name', 'test-project', '-y'], {
        cwd: tempDir,
        reject: false,
      });
      ...
      expect(existsSync(join(tempDir, 'guidelines', 'AGENT_EDITING_INSTRUCTIONS.md'))).toBe(true);
      expect(existsSync(join(tempDir, 'guidelines', 'BRANCHING_STRATEGY.md'))).toBe(true);
      expect(existsSync(join(tempDir, 'guidelines', 'TESTING_STRATEGY.md'))).toBe(true);
    ...
    it('creates GitHub Actions workflow for GitHub repos', async () => {
      ...
      await execa('git', ['init'], { cwd: tempDir });
      await execa('git', ['remote', 'add', 'origin', 'git@github.com:test/repo.git'], { cwd: tempDir });
      await writeFile(join(tempDir, 'package.json'), JSON.stringify({ 
        name: 'test-project',
        scripts: { lint: 'eslint .', test: 'vitest' }
      }));

      await execa('node', [cliPath, 'init', '--preset', 'library', '--name', 'test-lib', '-y'], {
        cwd: tempDir,
        reject: false,
      });
  ...
  describe('configuration output', () => {
    it('writes correct scopes from preset', async () => {
      ...
      await execa('node', [cliPath, 'init', '--preset', 'api', '--name', 'my-api', '-y'], {
        cwd: tempDir,
        reject: false,
      });

      const configContent = await readFile(join(tempDir, 'workflow.config.json'), 'utf-8');
  ...
  describe('hooks installation', () => {
    it('creates hooks directory in git repository', async () => {
      ...
      await execa('node', [cliPath, 'init', '--preset', 'saas', '--name', 'test', '-y'], {
        cwd: tempDir,
        reject: false,
      });
  ...
  describe('preset loading', () => {
    it('handles custom preset gracefully', async () => {
      ...
      const { exitCode } = await execa('node', [cliPath, 'init', '--preset', 'custom', '--name', 'custom-project', '-y'], {
        cwd: tempDir,
        reject: false,
      });
