...
async function addScopeAction(opts: ConfigOptions): Promise<void> {
  ...
  if (!existsSync(configPath)) {
    console.error(
      chalk.red("No workflow.config.json found. Run: workflow init"),
    );
    process.exit(1);
  }
  ...
  const response = await prompts([
    ...
    },
    {
      ...
      message:
        "Mandatory guidelines (comma-separated, or press enter to skip):",
      initial: "",
    },
  ]);
  ...
  if (newScope.mandatoryGuidelines) {
    console.log(
      chalk.gray(`  Guidelines: ${newScope.mandatoryGuidelines.join(", ")}`),
    );
  }
}

async function removeScopeAction(
  name: string,
  opts: ConfigOptions,
): Promise<void> {
  const cwd = opts.cwd || process.cwd();
...
async function listConfigAction(
  type: string,
  opts: ConfigOptions,
): Promise<void> {
  const cwd = opts.cwd || process.cwd();
  ...
  if (type === "scopes" || type === "all") {
    ...
    if (config.scopes.length === 0) {
      ...
      config.scopes.forEach((scope, index) => {
        ...
        console.log(
          chalk.gray(`     Types: ${scope.allowedTypes?.join(", ") || "all"}`),
        );
